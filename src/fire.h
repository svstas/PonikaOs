...
#define MAT_W   48          /* Size (columns) of entire matrix */
#define MAT_H   16          /* and rows */
#if defined(ESP32) || defined(ESP8266)
#define MAT_PIN 14           /* Data for matrix on D3 on ESP8266 */
#else
#define MAT_PIN 6           /* Data for matrix on pin 6 for Arduino/other */
#endif
#undef  MAT_ROW_MAJOR       /* define if matrix is column-major (that is pixel 1 is in the same column as pixel 0) */
#define  MAT_COL_MAJOR       /* define if matrix is column-major (that is pixel 1 is in the same column as pixel 0) */
//#define MAT_LEFT             /* define if matrix 0,0 is in top row of display; undef if bottom */
#define MAT_BOTTOM            /* define if matrix 0,0 is on left edge of display; undef if right */
#define MAT_ZIGZAG          /* define if matrix zig-zags ---> <--- ---> <---; undef if scanning ---> ---> ---> */

#define BRIGHT 160           /* brightness; min 0 - 255 max -- high brightness requires a hefty power supply! Start low! */
#define FPS 20              /* Refresh rate */
...
#define  PANEL_BOTTOM   1        /* define if first panel is upper-left */
